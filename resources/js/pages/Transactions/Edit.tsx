...
import { ArrowLeft, Minus, Package, Plus, Search, ShoppingCart, Trash2, User, Edit as EditIcon } from 'lucide-react';
...
export default function Edit() {
    const { customers, products, transaction } = usePage().props as unknown as {
        customers: Customer[];
        products: Product[];
        transaction: Transaction;
    };

    const [selectedCustomer, setSelectedCustomer] = useState<number | null>(transaction.customer_id);
    const [items, setItems] = useState<TransactionItem[]>(transaction.items || []);
    const [productSearch, setProductSearch] = useState('');
    const [customerSearch, setCustomerSearch] = useState(transaction.customer ? getCustomerName(transaction.customer) : '');
    ...
    // Filter products based on search - exclude already selected items
    ...
    const submitTransaction = () => {
        ...
        router.put(`/transactions/${transaction.id}`, data, {
            ...
            onError: (errors) => {
                console.error('Transaction update errors:', errors);
                setProcessing(false);
            },
        });
    };
                ...
                <div className="rounded-xl border border-sidebar-border/70 bg-white p-6 dark:border-sidebar-border dark:bg-sidebar-accent">
                    ...
                    <div className="mb-6 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Edit Transaction #{transaction.id}</h1>
                    ...
                    <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
                            ...
                            <div className="rounded-lg bg-gray-50 p-4 dark:bg-gray-800/30">
                                ...
                                {items.length === 0 ? (
                                    <div className="py-8 text-center text-gray-500">
                                        ...
                                        <p>No items in transaction. Add products above.</p>
                                    </div>
                                ) : (
                        ...
                        <div className="sticky top-4 h-fit rounded-lg bg-gray-50 p-4 dark:bg-gray-800/30">
                            ...
                            <div className="mb-4 rounded-lg bg-blue-50 p-3 dark:bg-blue-900/20">
                                <h4 className="mb-2 text-sm font-medium text-blue-800 dark:text-blue-200">Original Total</h4>
                                <div className="text-lg font-semibold text-blue-600 dark:text-blue-400">{formatCurrency(transaction.total)}</div>
                            </div>
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="border-t border-gray-200 pt-3 dark:border-gray-600">
                                    <div className="flex justify-between text-lg font-semibold">
                                        <span className="text-gray-900 dark:text-gray-100">New Total:</span>
                                ...
                                {total !== transaction.total && (
                                    <div className="flex justify-between text-sm">
                                        ...
                                        <span
                                            className={`font-mono ${total > transaction.total ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'}`}
                                        >
                                            {total > transaction.total ? '+' : ''}
                                            {formatCurrency(total - transaction.total)}
                                        </span>
                                    </div>
                                )}
                            </div>
